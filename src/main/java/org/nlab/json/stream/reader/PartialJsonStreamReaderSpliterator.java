...
import static org.nlab.json.stream.context.token.ObjectToken.KEY_UNINIT;
    ...
    try {
      ...
      if (jsonMatcherReader.getStreamContext().getElements().size() == depth) {
        ...
        if (token instanceof ObjectToken) {
          if (((ObjectToken) token).getKey().equals(key) || KEY_UNINIT.equals(key)) {
            return true;
          }
        ...
        eoi = true;
        return false;
      } else if (jsonMatcherReader.getStreamContext().getElements().size() < depth) {
        eoi = true;
        return false;
      }
